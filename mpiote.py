...
def read_cropped_image(p, augment):
    ...
    r = affine_transform(r, matrix, offset, output_shape=img_shape[:-1], order=1, mode='constant',
                           cval=np.average(r))
    g = affine_transform(g, matrix, offset, output_shape=img_shape[:-1], order=1, mode='constant',
                           cval=np.average(g))
    b = affine_transform(b, matrix, offset, output_shape=img_shape[:-1], order=1, mode='constant',
                           cval=np.average(b))
...
def build_model(lr, l2, activation='sigmoid'):
    ...
    x = BatchNormalization()(x)
    x = Conv2D(512, (1, 1), activation='relu', **kwargs)(x)  # 12x12x384
    for _ in range(4):
        x = subblock(x, 96, **kwargs)

    x = MaxPooling2D((2, 2), strides=(2, 2))(x)  # 6x6x384
    x = BatchNormalization()(x)
    x = Conv2D(768, (1, 1), activation='relu', **kwargs)(x)  # 6x6x512
    for _ in range(4):
...
class TrainingData(Sequence):
    def __init__(self, score, steps=1000, batch_size=128):
        """
...
def compute_score(verbose=1):
    ...
    """
    features = branch_model.predict_generator(FeatureGen(train, verbose=verbose), max_queue_size=10, workers=10,
                                              verbose=0)
    score = head_model.predict_generator(ScoreGen(features, verbose=verbose), max_queue_size=30, workers=10, verbose=0)
...
def make_steps(step, ampl):
    ...
    history = model.fit_generator(
        ...
        initial_epoch=steps, epochs=steps + step, max_queue_size=64, workers=10, verbose=1).history
    steps += step
...
prepare_submission(0.95, 'submission.csv')
toc = time.time()

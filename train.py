...
def build_model(lr, l2, activation='sigmoid'):
    ...
    branch_model = multi_gpu_model(Model(inp, x), gpus=2)
    ...
    head_model = multi_gpu_model(Model([xa_inp, xb_inp], x, name='head'), gpus=2)
    ...
    x = head_model([xa, xb])
    model = multi_gpu_model(Model([img_a, img_b], x), gpus=2)
...
def compute_score(verbose=1):
    ...
    """
    features = branch_model.predict_generator(FeatureGen(train, verbose=verbose), max_queue_size=30, workers=10,
                                              verbose=0)
...
def make_steps(step, ampl):
    ...
    history = model.fit_generator(
        ...
        initial_epoch=steps, epochs=steps + step, max_queue_size=256, workers=20, verbose=1).history
    steps += step
